...
import { createCookiesOptions, type Cookie, type CookiesOptions } from "./security/cookie.js";
...
interface NewSession {
   ...
   */
  user?: AponiaAuth.User

  /**
   * The new access token.
   */
  accessToken: AponiaAuth.AccessToken
...
export interface SessionConfig {
  ...
  cookieOptions: Pick<CookiesOptions, 'accessToken' | 'refreshToken'>

  createSession?: (user: AponiaAuth.User) => Awaitable<NewSession | Nullish>;

  getAccessTokenUser: (session: AponiaAuth.AccessToken) => Awaitable<AponiaAuth.User | Nullish>;

  handleRefresh?: (refreshToken: AponiaAuth.RefreshToken) => Awaitable<NewSession | Nullish>;

  onInvalidateAccessToken?: (
    accessToken: AponiaAuth.AccessToken,
    refreshToken: AponiaAuth.RefreshToken | Nullish,
...
export class SessionManager {
  ...
  constructor(config: SessionUserConfig) {
    ...
    this.config = defu(config, {
      ...
      cookieOptions,
      getAccessTokenUser: (accessToken: AponiaAuth.AccessToken) => accessToken,
    })
  }
  ...
  async decodeTokens(tokens: { accessToken?: string, refreshToken?: string }) {
    ...
    const refreshTokenData = await asPromise(
      this.config.jwt.decode<AponiaAuth.RefreshToken>({ secret: this.config.secret, token: tokens.refreshToken })
    ).catch(e => {
      console.log('Error decoding access token', e)
    })
   ...
   */
  async getUserFromRequest(request: AponiaAuth.InternalRequest): Promise<AponiaAuth.User | Nullish> {
   ...
   */
  async handleRequest(request: AponiaAuth.InternalRequest): Promise<AponiaAuth.InternalResponse> {
    const accessToken = request.cookies[this.config.cookieOptions.accessToken.name]
   ...
   * Log a user out.
   */
  async logout(request: Request): Promise<AponiaAuth.InternalResponse> {
