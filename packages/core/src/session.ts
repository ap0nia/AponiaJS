...
export class SessionManager {
  ...
  async decodeTokens(tokens: { accessToken?: string, refreshToken?: string }) {
    const accessTokenData = await asPromise(
      this.config.jwt.decode<Aponia.AccessToken>({ secret: this.config.secret, token: tokens.accessToken })
    ).catch(e => {
      console.log('Error decoding access token', e)
    })

    const refreshTokenData = await asPromise(
      this.config.jwt.decode<Aponia.RefreshToken>({ secret: this.config.secret, token: tokens.refreshToken })
    ).catch(e => {
      console.log('Error decoding access token', e)
    })
